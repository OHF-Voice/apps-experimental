    ...
    def __prep__(self):
        ...
        if (self.pkl is not None) and (os.path.isfile(self.pkl)):
            ...
            for target in self.target_list:
                ...
                if self.testset_only:
                    ...
                    with open(test_list, "r") as f:
                        ...
                        wav_list = [
                            os.path.join(self.wav_dir, x.strip()) for x in wav_list
                        ]
                        wav_list = [x for x in wav_list if target == x.split("/")[-2]]
                else:
                    wav_list = [
                        str(x)
                        for x in Path(os.path.join(self.wav_dir, target)).rglob("*.wav")
                    ]
                for wav in wav_list:
            ...
            self.data = self.data.append(
                pd.DataFrame.from_dict(target_dict, "index"), ignore_index=True
            )
...
if __name__ == "__main__":
    dataloader = GoogleCommandsDataloader(
        2048,
        testset_only=True,
        pkl="/home/DB/google_speech_commands/google_testset.pkl",
        features="g2p_embed",
    )
