...
from tensorflow.keras.utils import OrderedEnqueuer, Sequence
...
class LibriPhraseDataloader(Sequence):
    ...
    def __prep__(self):
        if self.train:
            ...
            for noise in noise_list:
                ...
                assert (
                    fs == self.fs
                ), ">> Error : Un-match sampling freq.\n{} -> {}".format(noise, fs)
                data = data.astype(np.float32) / 32768.0
                data = (data / np.max(data)) * 0.5
                self.noise = np.append(self.noise, data)

        self.data = pd.DataFrame(
            columns=["wav_label", "wav", "text", "duration", "label", "type"]
        )
        ...
        if (self.pkl is not None) and (os.path.isfile(self.pkl)):
            ...
            for db in self.train_csv if self.train else self.test_csv:
                csv_list = [
                    str(x) for x in Path(self.csv_dir).rglob("*" + db + "*word*")
                ]
                for n_word in csv_list:
                    ...
                    self.data = self.data.append(
                        anc_pos.rename(
                            columns={
                                y: x for x, y in zip(self.data.columns, anc_pos.columns)
                            }
                        ),
                        ignore_index=True,
                    )
                    self.data = self.data.append(
                        anc_neg.rename(
                            columns={
                                y: x for x, y in zip(self.data.columns, anc_neg.columns)
                            }
                        ),
                        ignore_index=True,
                    )
                    self.data = self.data.append(
                        com_pos.rename(
                            columns={
                                y: x for x, y in zip(self.data.columns, com_pos.columns)
                            }
                        ),
                        ignore_index=True,
                    )
                    self.data = self.data.append(
                        com_neg.rename(
                            columns={
                                y: x for x, y in zip(self.data.columns, com_neg.columns)
                            }
                        ),
                        ignore_index=True,
                    )
            ...
            self.data["wav"] = self.data["wav"].apply(
                lambda x: os.path.join(self.wav_dir, x)
            )
            # g2p & p2idx by g2p_en package
            print(">> Convert word to phoneme")
            self.data["phoneme"] = self.data["text"].apply(
                lambda x: self.g2p(re.sub(r"[^a-zA-Z0-9]+", " ", x))
            )
            print(">> Convert speech word to phoneme")
            self.data["wav_phoneme"] = self.data["wav_label"].apply(
                lambda x: self.g2p(re.sub(r"[^a-zA-Z0-9]+", " ", x))
            )
            print(">> Convert phoneme to index")
            self.data["pIndex"] = self.data["phoneme"].apply(
                lambda x: [self.p2idx[t] for t in x]
            )
            print(">> Convert speech phoneme to index")
            self.data["wav_pIndex"] = self.data["wav_phoneme"].apply(
                lambda x: [self.p2idx[t] for t in x]
            )
            print(">> Compute phoneme embedding")
            self.data["g2p_embed"] = self.data["text"].apply(
                lambda x: self.g2p.embedding(x)
            )
            print(">> Calucate Edit distance ratio")
            self.data["dist"] = self.data.apply(
                lambda x: Levenshtein.ratio(
                    re.sub(r"[^a-zA-Z0-9]+", " ", x["wav_label"]),
                    re.sub(r"[^a-zA-Z0-9]+", " ", x["text"]),
                ),
                axis=1,
            )
        ...
        self.maxlen_t = int(
            (int(self.data["text"].apply(lambda x: len(x)).max() / 10) + 1) * 10
        )
        self.maxlen_a = int(
            (int(self.data["duration"].values[-1] / 0.5) + 1) * self.fs / 2
        )
        self.maxlen_l = int(
            (int(self.data["wav_label"].apply(lambda x: len(x)).max() / 10) + 1) * 10
        )
    ...
    def __getitem__(self, idx):
        ...
        # load inputs
        batch_x = [
            np.array(wavfile.read(self.wav_list[i])[1]).astype(np.float32) / 32768.0
            for i in indices
        ]
        if self.features == "both":
            ...
            if self.features == "phoneme":
                ...
                batch_y = [
                    np.array(self.emb_list[i]).astype(np.float32) for i in indices
                ]
        # load outputs
        ...
        if self.edit_dist:
            batch_d = [
                np.array([self.dist_list[i]]).astype(np.float32) for i in indices
            ]

        # padding and masking
        pad_batch_x = pad_sequences(
            np.array(batch_x),
            maxlen=self.maxlen_a,
            value=0.0,
            padding="post",
            dtype=batch_x[0].dtype,
        )
        if self.features == "both":
            pad_batch_p = pad_sequences(
                np.array(batch_p),
                maxlen=self.maxlen_t,
                value=0.0,
                padding="post",
                dtype=batch_p[0].dtype,
            )
            pad_batch_e = pad_sequences(
                np.array(batch_e),
                maxlen=self.maxlen_t,
                value=0.0,
                padding="post",
                dtype=batch_e[0].dtype,
            )
        else:
            pad_batch_y = pad_sequences(
                np.array(batch_y),
                maxlen=self.maxlen_t,
                value=0.0,
                padding="post",
                dtype=batch_y[0].dtype,
            )
        pad_batch_z = pad_sequences(
            np.array(batch_z), value=0.0, padding="post", dtype=batch_z[0].dtype
        )
        pad_batch_l = pad_sequences(
            np.array(batch_l),
            maxlen=self.maxlen_l,
            value=0.0,
            padding="post",
            dtype=batch_l[0].dtype,
        )
        pad_batch_t = pad_sequences(
            np.array(batch_t),
            maxlen=self.maxlen_t,
            value=0.0,
            padding="post",
            dtype=batch_t[0].dtype,
        )
        if self.edit_dist:
            pad_batch_d = pad_sequences(
                np.array(batch_d), value=0.0, padding="post", dtype=batch_d[0].dtype
            )

        # Noisy option
        if self.train:
            batch_x_noisy = [self._mixing_snr(x) for x in batch_x]
            pad_batch_x_noisy = pad_sequences(
                np.array(batch_x_noisy),
                maxlen=self.maxlen_a,
                value=0.0,
                padding="post",
                dtype=batch_x_noisy[0].dtype,
            )
            ...
            if dataloader.train:
                ...
                if dataloader.features == "both":
                    if dataloader.edit_dist:
                        ...
                        pad_batch_x, pad_batch_p, pad_batch_e, pad_batch_z = dataloader[
                            i
                        ]
                        yield pad_batch_x, pad_batch_p, pad_batch_e, pad_batch_z
                else:
                    if dataloader.edit_dist:
                        pad_batch_x, pad_batch_y, pad_batch_z, pad_batch_d = dataloader[
                            i
                        ]
        ...
        if dataloader.features == "both":
            data_dataset = tf.data.Dataset.from_generator(
                ...
                output_signature=(
                    ...
                    tf.TensorSpec(
                        shape=(None, dataloader.maxlen_t, 256), dtype=tf.float32
                    ),
            ...
            data_dataset = tf.data.Dataset.from_generator(
                ...
                output_signature=(
                    ...
                    tf.TensorSpec(
                        shape=(
                            (None, dataloader.maxlen_t)
                            if dataloader.features == "phoneme"
                            else (None, dataloader.maxlen_t, 256)
                        ),
                        dtype=(
                            tf.int32 if dataloader.features == "phoneme" else tf.float32
                        ),
                    ),
                    tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
                    tf.TensorSpec(shape=(None, dataloader.maxlen_l), dtype=tf.int32),
        ...
        if dataloader.features == "both":
            if dataloader.edit_dist:
                data_dataset = tf.data.Dataset.from_generator(
                    ...
                    output_signature=(
                        tf.TensorSpec(
                            shape=(None, dataloader.maxlen_a), dtype=tf.float32
                        ),
                        tf.TensorSpec(
                            shape=(None, dataloader.maxlen_t), dtype=tf.int32
                        ),
                        tf.TensorSpec(
                            shape=(None, dataloader.maxlen_t, 256), dtype=tf.float32
                        ),
...
if __name__ == "__main__":
    ...
    train_dataset = LibriPhraseDataloader(
        batch_size=GLOBAL_BATCH_SIZE,
        train=True,
        types="both",
        shuffle=True,
        pkl="/home/DB/LibriPhrase/data/train_both.pkl",
        features="g2p_embed",
    )
    test_dataset = LibriPhraseDataloader(
        batch_size=GLOBAL_BATCH_SIZE,
        train=False,
        edit_dist=True,
        types="both",
        shuffle=False,
        pkl="/home/DB/LibriPhrase/data/test_both.pkl",
        features="g2p_embed",
    )
