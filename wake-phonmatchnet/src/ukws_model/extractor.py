        ...
        if ("_keras_mask" in vars(Q)) and ("_keras_mask" in vars(V)):
            if Q._keras_mask is None:
                ...
                attn_mask = tf.expand_dims(
                    tf.cast(Q._keras_mask, tf.int32), -1
                ) * tf.expand_dims(tf.cast(V._keras_mask, tf.int32), 1)

        else:
            attn_mask = None

        attention_output, affinity_matrix = self.attn(
            Q, V, return_attention_scores=True, attention_mask=attn_mask
        )
